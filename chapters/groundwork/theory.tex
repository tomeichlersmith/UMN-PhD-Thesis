...
\todo[figure]{need to find bullet cluster figure}
...
The repulsive pressure within the plasma originates from particles interacting electromagnetically
with each other, so the oscillations would be disrupted by the presence of extra matter interacting
gravitationally but not interacting electromagnetically.
...
Weak Force (as described in \cref{chapter:intro}); thus, they are named \acp{wimp}.
This phase space was first searched due to its theoretical
simplicitity: no new forces, just an extra particle (or two) creating the clouds of \ac{dm} we see
today. Unfortunately, these searches have not found evidence of a \ac{wimp}-like
...
which is even weaker than the Weak Force. In comparison to the heavier \acp{wimp},
...
to decay \emph{back} into standard matter eventually (and in some models, rather quickly).
This is where we expand on the idea of a ``dark sector''.
In this description of \ac{dm}, we already have a dark photon representing some
...
\todo[refactor]{Refactor visible signature section. Put SIMP detail into HPS part and use initial
	section to allude to trade-offs between visible and invisible search strategies.}
...
products are observable by our detectors, this signature is called visible.\todo[clarify]{
	Clarify that even when $m_{A'} < 2m_\chi$, it can still be used produce the cosmological $\chi$.
	(i.e. virtual particles can deviate from their mass in the quantum realm.)
}
...
studied in this work.\todo[reword]{reword the visible signature paragraphs to include vanilla model
	as a reasonable visible signature to search for. Remind the reader that the quantum nature of
	virtual particles allow them to deviate from the mass.}
...
\todo[reword]{
	These theories propose new specific vertex rules for constructing the diagram
	and the vertex factors for calculations.
}
