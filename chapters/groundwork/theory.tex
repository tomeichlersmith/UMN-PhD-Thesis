...
\section{Dark Matter?}
\begin{todoenv}
DM is a brutally honest description of the present level of understanding.
\end{todoenv}
...
General Relativity (\gls{gr}) \cite{rotationcurve-predictions-2007} and the observations differ drastically from this
...
These Baryon Acoustic Oscillations (\gls{bao}) are imprinted on
our snapshot of this early-universe plasma, the Cosmic Microwave Background (\gls{cmb}), 
which we can measure with a high degree
...
of a dark matter particle. The \gls{cmb} observations are tied to very early on in the existence
...
a few criteria that must be met by a proposed dark matter candidate (\gls{dm}).
\begin{itemize}
...
more exluded after many years of searching. Below $m_p$, the \gls{dm} requires a new force
...
and positrons we can observe and is discussed in more detail in \cref{sec:hps-idm}. This dark sector model
