...
sensitivity estimate so they are left in the sample.
...
In this subsample, the expected signal was artificially scaled such that it would
...
In summary, this optimization procedure led to the following cuts.
\begin{itemize}
...
\end{itemize}
where
%$p_m$ and $y_{0,\min}^\mathrm{cut}$ are evaluated at the mass point $m_{V_D}$ we are testing,
...
These selections lead to 60\todo[update]{update with 100\% results after unblinding}
...
\todo[update]{Update comments on search results after unblinding.}
...
as well as previously within the selection optimization (shown in \cref{fig:hps-signal-yield:expected}).
The maximum signal yield allowed is calculated using the \ac{oim} \cite{yellin-oim:2002}
which leverages the differing shape between signal and background and allows for the scale
of the background to be unknown.
The \ac{oim} result is shown in \cref{fig:hps-signal-yield:allowed}.

\cref{fig:sensitivity} shows the sensitivity along with the exclusion contour drawn where the sensitivity
equals one enclosing the area where the expected yield exceeds what is allowed and is there for excluded
for both this L1L2 analysis and the L1L1 analysis for comparison.
