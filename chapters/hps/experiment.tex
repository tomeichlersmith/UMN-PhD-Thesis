...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\chapter{The Heavy Photon Search Experiment}
\label{chapter:hps:experiment}

The \acf{hps} is a fixed-target experiment currently installed at \ac{jlab},
which is focused on the search for short-length (few centimeter) decays of new particles.
...
The baseline focus for \ac{hps} is to search for rare interactions which
produce dark photons that return back to the normal sector after a brief lifetime.
This search is the origin for the name of the experiment and the primary
motivation for the design of the two key detector components described in
more detail below.
We will instead consider a model where some of the energy remains within
the dark sector.
%, attempting to utilize the same detector and data sample designed for loss-less search
...
interact with each other strongly in an analagous sense as the standard quarks. These \acp{simp}
\cite{simp-mechanism-2014,simp-pheno-2018} would then form
...
matter candidates and the ones we represent within our diagrams. A dark sector consisting of \acp{simp} 
would have a plethora of composite particles which we could produce; however of particular
...
\begin{figure}
  ...
  \caption{
    ...
    through \ac{simp} composite particle $V_D$ with the emission of a lighter \ac{simp} composite particle $\pi_D$.
    The two-particle vertex $V_D-A'$ is allowed since they share enough quantum properties to mix.
  }
  \label{fig:dark-brem-simp-decay}
\end{figure}

Importantly, \cref{fig:dark-brem-simp-decay} has two vertices where \ac{dm} and \ac{sm} particles
interact -- both of these two vertices carry with them the weak mixing factor $\epsilon$.
The first vertex where the $A'$ is produced suppresses the rate that this process occurs by a factor
of $\epsilon^2$ similar to the previous case with \ac{ldmx}, but the second vertex does not affect the rate.
Since, in this model and other visible-signature models, the dark matter being produced cannot
viably decay to other dark matter (because $m_{A'} < 2 m_\chi$ for example), this second vertex forces
a longer decay length.
\ac{hps} has been built to observe a beam whose rate is significantly high in order to help overcome not
only the low production rate but also the acceptance and efficiency of observing the electron and positron
decay products -- high enough to prevent sensitive detector elements from being placed directly within its path.

The observable outgoing particles of \cref{fig:dark-brem-simp-decay} can be mimicked by existing
\ac{sm} processes. The most prominent background process is trident production.
\cref{fig:trident} shows processes which produce the electron-positron pair through the radiation
of the virtual photon (\cref{fig:trident:radiative}) or through direct electromagnetic interaction with the
nucleus and a virtual lepton (\cref{fig:trident:bethe-heitler}).
The difference between a \ac{dm} signal process and this trident
production process is that the trident process is \emph{prompt}
-- i.e. the produced electron and positron originate from within the target.
The position of this vertex is a separation from the signal process
...
\ac{hps} is designed to focus on visible signatures of \ac{dm} like \acp{simp} with
...
\ac{hps} is situated within an alcove behind the CLAS-12 experiment
such that it can collect data whenever CLAS-12 is not operational.
...
the Hall B alcove, \ac{hps} is built in two halves that straddle the primary
...
shown as red rectangles.\footnote{%
  This diagram and the description within this thesis is focused
...
The \ac{svt} modules are readout by APV25 chips which report six samples separated by \qty{24}{\ns}.
These samples are only dumped to disk upon receiving a trigger signal from the calorimeter system
(\cref{sec:hps-ecal}).
