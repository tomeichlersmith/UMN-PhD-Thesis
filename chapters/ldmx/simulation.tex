...
a high-energy photon converts into a pair of muons within the \ac{ecal}.
...
nevertheless, we need to efficiently sample events that contain these background processes
...
showed that $T_F = 0.5E_\text{Beam}$ was a good choice for the di-muon process.
...
\begin{figure}
  \centering
  \includegraphics[width=0.5\textwidth]{figures/ldmx/simulation/dimuon-z.pdf}
  \caption{
    Longitudinal location ($z$) where the $\mu^+$ was produced within the simulation
    for a variety of biasing factors $B$.
    ...
    we eventually over-bias and lose access to the late tail of the distribution (gray).
  }
  \label{fig:dimuon-z}
\end{figure}
